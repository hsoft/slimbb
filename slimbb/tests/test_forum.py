...
from slimbb.models import Category, Forum, Topic, Post
...
class TestForum(TestCase):
    ...
    def test_forum_view(self):
        response = self.client.get(reverse('slimbb:forum', kwargs={'forum_id':
                                                            self.forum.pk}))
        self.assertEqual(response.status_code, 200)

    def test_topic_view(self):
        response = self.client.get(reverse('slimbb:topic', kwargs={'topic_id':
                                                            self.topic.pk}))
        self.assertEqual(response.status_code, 200)

    def test_create_topic(self):
        response = self.client.post(reverse('slimbb:add_topic', kwargs={'forum_id':
                                                            self.forum.pk}),
                                                            {'name': 'title',
                                                            'body': 'topic body'
                                                            }
